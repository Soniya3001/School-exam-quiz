...
async def generate_subjective(lesson_text, image_b64, test_class, subject, language, api_key, system_msg, selected_topic: str = '') -> str:
    ...
    - 2 questions x 4 marks = 8 marks
    """
    syllabus_context = get_syllabus(test_class, subject)

    if selected_topic:
        topic_constraint = (
            f"MANDATORY: ALL 10 questions MUST be from ONLY this chapter/topic: '{selected_topic}'.\n"
            f"Do NOT generate even a single question from any other chapter or topic.\n"
            f"Every question must directly relate to '{selected_topic}' concepts only.\n"
        )
    else:
        topic_constraint = f"Generate questions covering core topics of Class {test_class} {subject}.\n"

    syllabus_section = ""
    if syllabus_context:
        syllabus_section = (
            f"\nNCERT/CBSE OFFICIAL SYLLABUS for Class {test_class} {subject}:\n"
            f"{syllabus_context}\n"
            f"ALL questions must be strictly within this syllabus only.\n"
        )

    prompt = (
        f"You are a CBSE examiner creating a subjective question paper for Class {test_class} {subject}.\n"
        f"Language: {language}\n"
        f"{topic_constraint}"
        f"{syllabus_section}\n"
        "CREATE EXACTLY 10 questions as per CBSE pattern (20 marks total):\n"
        "- Section A: EXACTLY 4 questions x 1 mark (Very Short Answer - 1-2 sentences)\n"
        "- Section B: EXACTLY 4 questions x 2 marks (Short Answer - 3-5 sentences)\n"
        "- Section C: EXACTLY 2 questions x 4 marks (Long Answer - detailed explanation)\n\n"
        "CBSE QUALITY RULES:\n"
        "1. Section A: Definitions, state theorems, one-word or one-line answers.\n"
        "2. Section B: Explain concepts, short derivations, numerical examples.\n"
        "3. Section C: Proofs, long derivations, detailed solutions with steps.\n"
        "4. ALL questions from specified topic ONLY - no cross-chapter mixing.\n"
        "5. Questions must match CBSE board exam standard and style.\n\n"
        "Return ONLY this JSON (no other text):\n"
        '{"questions":[' 
        '{"q":"question","marks":1,"section":"A","expected_answer":"answer","keywords":["k1","k2"]},'
        ...
        ']}\n'
        "STRICT: 4 questions with section=A marks=1, 4 questions with section=B marks=2, 2 questions with section=C marks=4.\n"
    )
    if lesson_text:
        ...
        prompt += f"\nBase all questions strictly on NCERT textbook content for '{selected_topic or subject}' Class {test_class}."
    return prompt, system_msg
...
async def generate_questions_llm(
    ...
    test_type_extra = {"topic": selected_topic}
    if test_type == "subjective":
        prompt, system_msg = await generate_subjective(lesson_text, image_b64, test_class, subject, language, api_key, system_msg, selected_topic=test_type_extra.get('topic', ''))
    else:
        prompt, system_msg = await generate_mcq(lesson_text, image_b64, count, test_class, subject, language, difficulty, api_key, system_msg, selected_topic=test_type_extra.get('topic', ''))
...
# NCERT Chapter/Topic Lists — Class 6 to 10
NCERT_TOPICS = {
    ...
    "10|Hindi": ["क्षितिज भाग 2 - गद्य", "क्षितिज भाग 2 - काव्य", "कृतिका भाग 2", "व्याकरण", "लेखन कौशल"],
    ...
    "6|English": ["Who Did Patrick's Homework?", "How the Dog Found Himself a New Master!", "Taro's Reward", "An Indian – American Woman in Space", "A Different Kind of School", "Who I Am", "Fair Play", "A Game of Chance", "Desert Animals", "The Banyan Tree", "A Tale of Two Birds", "The Friendly Mongoose", "The Shepherd's Treasure", "The Old-Clock Shop", "Tansen", "The Monkey and the Crocodile", "The Wonder Called Sleep", "A Pact with the Sun", "What Happened to the Reptiles?", "A Strange Wrestling Match"],
    "6|Hindi": ["वसंत - गद्य खंड", "वसंत - पद्य खंड", "दूर्वा", "बाल महाभारत", "व्याकरण - संज्ञा, सर्वनाम", "व्याकरण - विशेषण, क्रिया", "पत्र लेखन", "निबंध लेखन"],
    "6|Mathematics": ["Knowing Our Numbers", "Whole Numbers", "Playing with Numbers", "Basic Geometrical Ideas", "Understanding Elementary Shapes", "Integers", "Fractions", "Decimals", "Data Handling", "Mensuration", "Algebra", "Ratio and Proportion", "Symmetry", "Practical Geometry"],
    "6|Science": ["Food: Where Does It Come From?", "Components of Food", "Fibre to Fabric", "Sorting Materials into Groups", "Separation of Substances", "Changes Around Us", "Getting to Know Plants", "Body Movements", "The Living Organisms and Their Surroundings", "Motion and Measurement of Distances", "Light, Shadows and Reflections", "Electricity and Circuits", "Fun with Magnets", "Water", "Air Around Us", "Garbage In, Garbage Out"],
    "6|Social Science": ["What, Where, How and When?", "On The Trail of the Earliest People", "From Gathering to Growing Food", "In the Earliest Cities", "What Books and Burials Tell Us", "Kingdoms, Kings and an Early Republic", "New Questions and Ideas", "Ashoka, The Emperor Who Gave Up War", "Vital Villages, Thriving Towns", "Traders, Kings and Pilgrims", "New Empires and Kingdoms", "Buildings, Paintings and Books", "The Earth in the Solar System", "Globe: Latitudes and Longitudes", "Motions of the Earth", "Maps", "Major Domains of the Earth", "Major Landforms of the Earth", "Our Country – India", "India: Climate, Vegetation and Wildlife"],
    "7|English": ["Three Questions", "A Gift of Chappals", "Gopal and the Hilsa Fish", "The Ashes That Made Trees Bloom", "Quality", "Expert Detectives", "The Invention of Vita-Wonk", "Fire: Friend and Foe", "A Bicycle in Good Repair", "The Story of Cricket"],
    "7|Hindi": ["वसंत भाग 2 - गद्य", "वसंत भाग 2 - कविता", "दूर्वा भाग 2", "व्याकरण", "लेखन कौशल"],
    ...
    "9|English": ["The Fun They Had", "The Sound of Music", "The Little Girl", "A Truly Beautiful Mind", "The Snake and the Mirror", "My Childhood", "Packing", "Reach for the Top", "The Bond of Love", "Kathmandu", "If I Were You", "The Road Not Taken", "Wind", "Rain on the Roof", "The Lake Isle of Innisfree", "A Legend of the Northland", "No Men Are Foreign", "The Duck and the Kangaroo", "On Killing a Tree", "The Snake Trying", "A Slumber Did My Spirit Seal"],
    "9|Hindi": ["क्षितिज भाग 1 - गद्य", "क्षितिज भाग 1 - काव्य", "कृतिका भाग 1", "व्याकरण", "लेखन कौशल"],
    "9|Mathematics": ["Number Systems", "Polynomials", "Coordinate Geometry", "Linear Equations in Two Variables", "Introduction to Euclid's Geometry", "Lines and Angles", "Triangles", "Quadrilaterals", "Circles", "Heron's Formula", "Surface Areas and Volumes", "Statistics", "Probability"],
    "9|Science": ["Matter in Our Surroundings", "Is Matter Around Us Pure", "Atoms and Molecules", "Structure of the Atom", "The Fundamental Unit of Life", "Tissues", "Motion", "Force and Laws of Motion", "Gravitation", "Work and Energy", "Sound", "Why Do We Fall Ill", "Natural Resources", "Improvement in Food Resources"],
    "9|Social Science": ["The French Revolution", "Socialism in Europe and the Russian Revolution", "Nazism and the Rise of Hitler", "Forest Society and Colonialism", "Pastoralists in the Modern World", "India – Size and Location", "Physical Features of India", "Drainage", "Climate", "Natural Vegetation and Wildlife", "Population", "What is Democracy? Why Democracy?", "Constitutional Design", "Electoral Politics", "Working of Institutions", "Democratic Rights", "The Story of Village Palampur", "People as Resource", "Poverty as a Challenge", "Food Security in India"],
}


def get_topics(test_class: str, subject: str) -> list:
    import re as _re
    m = _re.search(r'(\d+)', test_class)
    if not m: return []
    cls_num = m.group(1)
    ...
    subj_norm = subj_map.get(subject.lower().strip(), subject.strip())
    return NCERT_TOPICS.get(f"{cls_num}|{subj_norm}", [])
