...
async def generate_subjective(lesson_text, image_b64, test_class, subject, language, api_key, system_msg, selected_topic: str = '') -> str:
    ...
    Total = 20 marks
    """
    # Get NCERT syllabus context
    syllabus_context = get_syllabus(test_class, subject)
    syllabus_note = (
        f"\n\nNCERT/CBSE SYLLABUS REFERENCE for Class {test_class} {subject}:\n{syllabus_context}\n"
        "IMPORTANT: Generate questions STRICTLY from the above syllabus chapters and topics only."
    ) if syllabus_context else f"\n\nFollow standard NCERT/CBSE syllabus for Class {test_class} {subject}."
