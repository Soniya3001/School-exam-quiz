...
import { CLASSES, SECTIONS, COLORS, RADII, SPACING, SUBJECTS } from "../../src/theme";
...
export default function TeacherDashboard() {
  ...
  const onGenerate = async () => {
    ...
    if (!testSection) return setErr("Please select a section.");
    ...
    try {
      await Api.teacherGenerate({
        ...
        test_class: `${testClass} - ${testSection}`,
        subject,
        language,
        difficulty,
      });
    ...
    <SafeAreaView style={s.safe}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <ScrollView contentContainerStyle={s.scroll}>
          ...
          <Card>
            <SectionTitle>1️⃣ Class, Section & Subject</SectionTitle>
            <View style={{ flexDirection: "row", gap: SPACING.sm }}>
              <View style={{ flex: 1 }}>
                <Picker label="Class" value={testClass} onChange={setTestClass} options={CLASSES} testID="teacher-class-picker" />
              </View>
              <View style={{ flex: 1 }}>
                <Picker label="Section" value={testSection} onChange={setTestSection} options={SECTIONS} testID="teacher-section-picker" />
              </View>
            </View>
            ...
            {/* Difficulty */}
            <Text style={[s.fieldLabel, { marginTop: SPACING.md }]}>Difficulty Level</Text>
            <View style={s.diffRow}>
              {DIFF_OPTIONS.map(({ d, label, emoji, desc, color }) => (
                <TouchableOpacity
                  ...
                  style={[
                    s.diffBtn,
                    difficulty === d && { borderColor: color, backgroundColor: color + "15" },
                  ]}
                >
            ...
            {testClass && testSection && subject && (
              <View style={{ flexDirection: "row", gap: 6, flexWrap: "wrap", marginTop: SPACING.md }}>
                <Pill>{testClass}</Pill>
                <Pill color={COLORS.accent}>{testSection}</Pill>
                <Pill color={COLORS.primary}>{subject}</Pill>
