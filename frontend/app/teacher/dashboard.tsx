...
export default function TeacherDashboard() {
  ...
  // Load topics when class or subject changes
  useEffect(() => {
    if (!testClass || !subject) { setAvailableTopics([]); setSelectedTopic(""); return; }
    Api.getTopics(testClass, subject)
      .then((res: any) => { setAvailableTopics(res.topics || []); setSelectedTopic(""); })
        ...
        <ScrollView contentContainerStyle={s.scroll}>
          ...
          <Card>
            ...
            {availableTopics.length > 0 && (
              <View style={{ marginBottom: SPACING.md }}>
                <Text style={s.fieldLabel}>📖 Select Chapter/Topic <Text style={{ color: COLORS.n500, fontWeight: "400" }}>(optional)</Text></Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 4 }}>
                  <View style={{ flexDirection: "row", gap: 8, paddingBottom: 4 }}>
                    <TouchableOpacity
                      onPress={() => setSelectedTopic("")}
                      style={[s.topicChip, !selectedTopic && s.topicChipActive]}
                    >
                      <Text style={[s.topicChipTxt, !selectedTopic && s.topicChipTxtActive]}>All Topics</Text>
                    </TouchableOpacity>
                    {availableTopics.map((t) => (
                      <TouchableOpacity key={t} onPress={() => setSelectedTopic(t)}
                        style={[s.topicChip, selectedTopic === t && s.topicChipActive]}>
                        <Text style={[s.topicChipTxt, selectedTopic === t && s.topicChipTxtActive]}>{t}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </ScrollView>
                {selectedTopic ? (
                  <Banner kind="success">📖 Topic: {selectedTopic}</Banner>
                ) : null}
              </View>
            )}
          ...
          {selectedTest && selectedTest.questions?.length > 0 && (
            <Card>
              ...
              <Button
                title="📄 View / Share Question Paper"
                variant="outline"
                onPress={() => router.push(`/teacher/paper?test_class=${encodeURIComponent(selectedTest.test_class.replace(/ /g, "_"))}`)}
                testID="view-paper-btn"
...
const s = StyleSheet.create({
  ...
  topicChip: {
    ...
    borderWidth: 1.5, borderColor: COLORS.n300, backgroundColor: COLORS.muted,
  },
  topicChipActive: { borderColor: COLORS.primary, backgroundColor: COLORS.primary },
  topicChipTxt: { fontSize: 12, fontWeight: "600", color: COLORS.n700 },
